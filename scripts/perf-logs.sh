...
ssh  -o "StrictHostKeyChecking=no"  -i $SSH_KEY ubuntu@$HORIZON_DOMAIN  "sudo docker exec data_horizon-db_1 psql -h localhost -U stellar horizon -c 'select transaction_hash, ledger_sequence from history_transactions where ledger_sequence >= 1000 and ledger_sequence <= 2000' " 1> /tmp/perf-tx-ledgers.txt
